...
- Review for correctness, clarity, simplicity, duplication, and unnecessary complexity. Prefer the simplest design that satisfies current requirements and follows established repository patterns. Look for unnecessary abstractions, indirection, redundant state, speculative flexibility, and avoidable special cases.
- For maintainability findings, identify a concrete maintenance burden and the smallest improvement that preserves required behavior. Fewer lines are not automatically simpler; similar-looking code alone does not justify an abstraction, and justified error handling must remain. Give each finding's location and consequence, prioritize by impact, and accept no findings as a valid result.
...
- Inspect the complete task-owned diff. As relevant, check scope, correctness, clarity, simplicity, duplication, unnecessary complexity, security, failure and resource handling, contracts, compatibility, concurrency, performance, tests, documentation, dependencies, generated files, schemas, and migrations.
